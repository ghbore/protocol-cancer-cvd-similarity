...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# load data
df <- lapply(snakemake@input, function (f){
    df <- readRDS(f)
    if ("dataset" %in% colnames(df)){
        ...
        mutate(
            df, 
            dataset = basename(f) |> str_remove("\\.[^.]+$")
        )
    }
}) |>
    bind_rows()

df <- filter(df, dataset %in% snakemake@params$datasets)
```

## Heatmap

```{r echo=FALSE, message=FALSE, warning=FALSE}
select(df, dataset, ID, NES) |>
    pivot_wider(ID, names_from = dataset, values_from = NES) |>
